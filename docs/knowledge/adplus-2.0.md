...
| # | 规则 | 状态 | 来源 | 代码位置 |
| --- | --- | --- | --- | --- |
| 1 | Standard / Plus 电源盒：最大支持 **1 路外接 IPC + 1 路 AHD** | ✅ 已确认 | 产品方案（用户 2026-07-10 口头确认） | `app.js: cameraCapacityRule()` |
| 2 | Power Box Max：最大 **1 路 IPC + 3 路 AHD**，或 **4 路 AHD（不带 IPC）** | ⚠️ 推断自代码 | 代码反推 | `app.js: cameraCapacityRule()` |
| 3 | CA46（AHD 摄像头，SKU 5151053100007）**只能搭配 PBM** | ⚠️ 推断自代码 | 代码反推 | `customCatalog.accessories → allowedPowerBoxes` |
| 4 | B2 声光报警器：**只能搭配 PBM**，最多 4 个，必须带转接件（row 45） | ⚠️ 推断自代码 | 代码反推 | `customCatalog.optionals → b2` |
| 5 | B3 报警器：**只能搭配 PBM**，最多 4 个，延长线为必配且不可换 | ⚠️ 推断自代码 | 代码反推 | `customCatalog.optionals → b3` |
| 6 | Micro SD 卡最多选 2 张 | ⚠️ 推断自代码 | 代码反推 | `customCatalog.optionals → micro_sd` |
| 7 | R-Watch 与 **Standard 电源盒 + CA42 拖挂转接** 的组合互斥（选了后者不能选 R-Watch） | ⚠️ 推断自代码 | 代码反推 | `app.js: ca42TrailerAdapterBlocksRwatch()` |
| 8 | 散线接线：Standard 电源盒内置；PBP / PBM 需要单独散线物料 | ⚠️ 推断自代码 | 代码反推 | `customCatalog.wiringModes → loose` |
| 9 | 16PIN OBD：PBP / PBM 套装默认包含 16PIN 电源延长线 | ⚠️ 推断自代码 | 代码反推 | `customCatalog.wiringModes → 16pin` |
...
见 `catalog-data.js`（AD Plus 2_0 产品线）及 `app.js: SKU_LIBRARY`。
